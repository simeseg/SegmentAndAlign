...
class icp_GN():
    def __init__(self, grid_size, distance_threshold, iterations, mode, animate, model, scene):
        ...
        self.fig.suptitle("point-to-{}-GN".format(self.mode))
        #display 
        self.subplot = 1
        #plot1
        self.ax1.set_xlim([self.min[0],self.max[0]])
        self.ax1.set_xlabel('x')
        self.ax1.set_ylabel('y')
        self.ax1.set_zlabel('z')
        utils.axisEqual3D(self.ax1)
        self.static1 = self.ax1.scatter(self.staticPointCloud[0,:], self.staticPointCloud[1,:], self.staticPointCloud[2,:], marker='.',color = "blue")
        self.ax1.scatter(self.dynamicPointCloud[0,:], self.dynamicPointCloud[1,:], self.dynamicPointCloud[2,:], marker='.', color = "red")
        self.dynamic1 = self.ax1.scatter(self.dynamicPointCloud[0,:], self.dynamicPointCloud[1,:], self.dynamicPointCloud[2,:], marker='.', color = "green")
        self.label1 = self.ax1.text(-0.5, -0.5, 50, "Step = 0  Error: %.2d"%(self.error))
        self.lines = []
        self.projs = []
        
        #plot2
        self.ax2.set_xlim([self.min[0],self.max[0]])
        self.ax2.set_xlabel('x')
        self.ax2.set_ylabel('y')
        self.ax2.set_zlabel('z')
        utils.axisEqual3D(self.ax2)
        self.static2 = self.ax2.scatter(self.staticPointCloud[0,:], self.staticPointCloud[1,:], self.staticPointCloud[2,:], marker='.',color = "blue")
        self.ax2.scatter(self.dynamicPointCloud[0,:], self.dynamicPointCloud[1,:], self.dynamicPointCloud[2,:], marker='.', color = "red")
        self.dynamic2 = self.ax2.scatter(self.dynamicPointCloud[0,:], self.dynamicPointCloud[1,:], self.dynamicPointCloud[2,:], marker='.', color = "green")
        self.label2 = self.ax2.text(-0.5, -0.5, 50, "Step = 0  Error: %.2d"%(self.error))
        self.lines = []
        self.projs = []
    ...
    def alignment_left(self, R):
        ...
        self.dynamic1._offsets3d = (points[0,:], points[1,:], points[2,:])
    ...
    def alignment_right(self, R):
        ...
        #initialize start using eigenvalue decomp
        points = R@(np.asarray(self.scene.points)).transpose(1,0) 
        self.dynamic2._offsets3d = (points[0,:], points[1,:], points[2,:])
    ...
    def update_point_to_point(self, framenumber):
        ...
        #update plots
        if self.subplot == 1:
            self.label1.set_text("Step = %.2d  Error = %.2f"%(framenumber, chi/self.numRandomSamples))
            self.dynamic1._offsets3d = (P[0,:], P[1,:], P[2,:])
            
        if self.subplot == 2:
            self.label2.set_text("Step = %.2d  Error = %.2f"%(framenumber, chi/self.numRandomSamples))
            self.dynamic2._offsets3d = (P[0,:], P[1,:], P[2,:])
    ...
    def eigenvalue_init(self):
        ...
        self.ax1.plot3D([0, end[0]], [0, end[1]], [0, end[2]], color = 'b' )
        self.ax2.plot3D([0, end[0]], [0, end[1]], [0, end[2]], color = 'b' )
        
        end = 50*v2
        self.ax1.plot3D([0, end[0]], [0, end[1]], [0, end[2]], color = 'r' )
        self.ax2.plot3D([0, end[0]], [0, end[1]], [0, end[2]], color = 'r' )
        ...
        self.ax1.plot3D([0, end[0]], [0, end[1]], [0, end[2]], color = 'g' )
        self.ax2.plot3D([0, -end[0]], [0, -end[1]], [0, -end[2]], color = 'g' )
        
        #plot rotation axis
        end = 50*ax.reshape(3)
        self.ax1.plot3D([0, end[0]], [0, end[1]], [0, end[2]], color = 'k' )
        self.ax2.plot3D([0, end[0]], [0, end[1]], [0, end[2]], color = 'k' )
